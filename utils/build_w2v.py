...
def build(train_x_seg_path, test_y_seg_path, test_seg_path, out_path=None, sentence_path='',
    ...
    w2v = （one line）
    """
    # 注意min_count要传入word2vec
    w2v = Word2Vec(LineSentence(sentence_path), size = 256, sg = 1, workers= 4, min_count = min_count)
...
if __name__ == '__main__':
    build('{}/datasets/train_set.seg_x_dev.txt'.format(BASE_DIR),
          '{}/datasets/train_set.seg_y_dev.txt'.format(BASE_DIR),
          '{}/datasets/test_set.seg_x_dev.txt'.format(BASE_DIR),
          out_path='{}/datasets/word2vec_dev.txt'.format(BASE_DIR),
          sentence_path='{}/datasets/sentences_dev.txt'.format(BASE_DIR))
    print("finish")
