...
def build_vocab(items, sort=True, min_count=0, lower=False):
    ...
    reverse_vocab应该是index,word的
    """
    vocab = [(key,result.index(key)) for key in result]
    reverse_vocab = [(result.index(key), key) for key in result]

    return vocab, reverse_vocab


if __name__ == '__main__':
    lines = read_data('{}/datasets/train_set.seg_x_dev.txt'.format(BASE_DIR),
                      '{}/datasets/train_set.seg_y_dev.txt'.format(BASE_DIR),
                      '{}/datasets/test_set.seg_x_dev.txt'.format(BASE_DIR))
    vocab, reverse_vocab = build_vocab(lines)
    print("Vocaburay size is {}".format(len(vocab)))
    save_word_dict(vocab, '{}/datasets/vocab_dev.txt'.format(BASE_DIR))
    print("Finish")